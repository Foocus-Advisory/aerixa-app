-- V1.22__refine_invitation_mail_typography.sql
-- Affine la typographie des mails d'invitation avec une stack Grift-first

UPDATE auth.mail_templates AS t
SET
    html_content = $invitation_refined_html$
<!doctype html>
<html lang="fr">
<head>
  <meta charset="UTF-8" />
...
<body style="margin:0;padding:0;background:#07142f;background-image:radial-gradient(circle at 18% 20%, rgba(116,72,255,0.28), transparent 36%),radial-gradient(circle at 85% 82%, rgba(17,195,214,0.18), transparent 40%);font-family:'Grift','Aptos','Segoe UI','Helvetica Neue',Arial,sans-serif;color:#e8eefb;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;padding:26px 14px;background:transparent;">
    <tr>
      <td align="center">
...
$invitation_refined_html$,
    text_content = 'Bonjour $${USER_NAME},

  Votre invitation $${SYSTEM_NAME} est prete.
  Definissez votre mot de passe ici : $${INVITATION_LINK}

Si vous n''etes pas a l''origine de cette demande, ignorez cet email.
  Support : $${SUPPORT_EMAIL}',
    preview = 'Invitation AERIXA au style typographique affinee',
