...
@WebMvcTest(
        controllers = AcademicLevelsController.class,
...
class AcademicLevelsControllerSecurityTest {
    ...
    private static final String ACADEMIC_LEVEL_ID = "33333333-3333-3333-3333-333333333333";
    ...
    @Test
    @WithMockUser(username = ACTOR_ID, authorities = {"academic_levels:list"})
    void listWithPermissionReturnsOk() throws Exception {
        given(academicLevelService.list(any(UUID.class), any(UUID.class), anyString())).willReturn(List.of());

        mockMvc.perform(get("/api/v1/academic-levels").param("establishmentId", ESTABLISHMENT_ID))
                .andExpect(status().isOk());
    }

    @Test
    @WithMockUser(username = ACTOR_ID, authorities = {"academic_levels:create"})
    void createWithPermissionReturnsCreated() throws Exception {
        given(academicLevelService.create(any(UUID.class), any(), anyString()))
                .willReturn(AcademicLevelResponse.builder().id(UUID.fromString(ACADEMIC_LEVEL_ID)).code("BTS1").label("BTS 1").rankOrder(1).active(true).build());

        mockMvc.perform(post("/api/v1/academic-levels")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"establishmentId\":\"" + ESTABLISHMENT_ID + "\",\"code\":\"BTS1\",\"label\":\"BTS 1\",\"rankOrder\":1}"))
                .andExpect(status().isCreated());
    }

    @Test
    @WithMockUser(username = ACTOR_ID, authorities = {"academic_levels:read"})
