            ...
            <Card className="border-border/60 bg-card/70 shadow-sm">
              <CardHeader className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
                ...
                <Badge className="w-fit" variant="outline">
                  ...
                  {locale === "fr" ? "Section : Étapes du tunnel" : "Section: Funnel stages"}
                </Badge>
              </CardHeader>
            </Card>
                              ...
                              items={[
                                {
                                  label: t.tooltipView,
                                  icon: Eye,
                                  onClick: () => router.push(`/dashboard/funnel-stages/${item.id}?establishmentId=${effectiveEstId}`),
                                },
                                ...(canUpdate ? [{
                                  label: t.tooltipEdit,
                                  icon: Pencil,
                  ...
                  {query.isLoading ? (
                    ...
                    <div key={item.id} className={`rounded-xl border border-border/70 bg-background/70 p-4 ${selectedIds.has(item.id) ? "border-primary/40 bg-primary/5" : ""}`}>
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Button size="sm" variant="outline" className="h-8 rounded-lg px-2 text-xs" onClick={() => router.push(`/dashboard/funnel-stages/${item.id}?establishmentId=${effectiveEstId}`)}>
                  ...
                  <div className="grid gap-3 overflow-x-auto pb-2 md:grid-flow-col md:auto-cols-[minmax(260px,1fr)]">
                    {query.isLoading ? (
                      ...
                      STAGE_TYPES.map((type) => {
                        ...
                        return (
                          <div key={type} className="flex min-w-0 flex-col gap-2 rounded-xl border border-border/70 bg-background/50 p-3">
                            ...
                            <div className="flex flex-col gap-2">
                              {stageItems.length === 0 ? (
                                ...
                                stageItems
                                  ...
                                  .map((item) => (
                                    <div key={item.id} className="rounded-lg border border-border/70 bg-card/80 p-3 shadow-sm">
                                      ...
                                      <div className="flex flex-wrap gap-2">
                                        <Button size="sm" variant="outline" className="h-7 rounded-lg px-2 text-xs" onClick={() => router.push(`/dashboard/funnel-stages/${item.id}?establishmentId=${effectiveEstId}`)}>
                                          <Eye className="mr-1 h-3 w-3" />{t.tooltipView}
                                        </Button>
                                        {canUpdate && (
                                          <Button size="sm" variant="outline" className="h-7 rounded-lg px-2 text-xs" onClick={() => openEditDialog(item)}>
                                            <Pencil className="mr-1 h-3 w-3" />{t.tooltipEdit}
                                          </Button>
                                        )}
                                      </div>
