...
export default function ProgramTrackLevelsPage() {
  ...
  const openViewDialog = (item: ProgramTrackLevelResponse) => {
    setFocusedItem(item);
    setDialogMode("view");
    setDialogOpen(true);
  };
  ...
  const t = {
    ...
    viewTitle: locale === "fr" ? "Détail de l'association" : "Association details",
    ...
    selectEstablishment: locale === "fr" ? "Sélectionner un établissement" : "Select an establishment",
    selectProgramTrack: locale === "fr" ? "Sélectionner une filière" : "Select a program track",
    selectAcademicLevel: locale === "fr" ? "Sélectionner un niveau" : "Select an academic level",
            ...
            <Card className="border-border/60 bg-card/70 shadow-sm">
              <CardHeader className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
                ...
                <Badge className="w-fit" variant="outline">
                  ...
                  {locale === "fr" ? "Section : Niveaux par filière" : "Section: Program track levels"}
                </Badge>
              </CardHeader>
            </Card>
      ...
      <Dialog open={dialogOpen && dialogMode === "view"} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{locale === "fr" ? "Informations complètes de l'association." : "Full association details."}</DialogDescription>
          </DialogHeader>
          {focusedItem && (
            <div className="grid gap-3 text-sm">
              <div className="rounded-lg border border-border/70 bg-background/70 p-3">
                <p className="text-xs text-muted-foreground">{t.fieldProgramTrack}</p>
                <p className="mt-1 font-medium">{trackLabel(focusedItem.programTrackId)}</p>
              </div>
              <div className="rounded-lg border border-border/70 bg-background/70 p-3">
                <p className="text-xs text-muted-foreground">{t.fieldAcademicLevel}</p>
                <p className="mt-1">{levelLabel(focusedItem.academicLevelId)}</p>
              </div>
              <div className="rounded-lg border border-border/70 bg-background/70 p-3">
                <p className="text-xs text-muted-foreground">{t.colStatus}</p>
                <div className="mt-1"><Badge variant={focusedItem.openForApplication ? "success" : "outline"}>{focusedItem.openForApplication ? t.open : t.closed}</Badge></div>
              </div>
